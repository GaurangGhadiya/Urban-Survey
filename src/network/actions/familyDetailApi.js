  ...
  return async (dispatch) => {
    try {
      const response = await axios.get(
        `/familyDetail?himParivarId=${encryptDataGet(JSON.stringify(parivarId))}&rationCardNo=${encryptDataGet(JSON.stringify(rationNo))}`,
        {}
      );
      let originalText = decryptData(response?.data?.data);
      dispatch(fetchFamiliesDetSuccess(originalText));
    } catch (error) {
