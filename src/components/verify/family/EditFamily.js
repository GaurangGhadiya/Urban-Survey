...
export default function EditFamily({ onsave, selectedFamily }) {
  ...
  useEffect(() => {
    ...
    if (municipality_reducer?.data) {
      const { data } = municipality_reducer || {};
            ...
            <FormControl fullWidth>
              ...
              >
                {districtList.map((option) => (
                  <MenuItem key={option.id} value={option.districtName}>
                    {option.districtName}
                  </MenuItem>
                ))}
              </Select>
          ...
          <FormControl fullWidth>
            ...
            >
              {municipalityList?.map((option) => (
                <MenuItem key={option?.id} value={option?.municipalName}>
